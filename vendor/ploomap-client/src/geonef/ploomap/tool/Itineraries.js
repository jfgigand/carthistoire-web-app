             ...
             [ geonef.jig.layout._Anchor, dijit._Templated, geonef.ploomap.MapBinding ],
{
  ...
  onRouteFound: function(directions) {
    ...
    , td2 = dc('td', { colspan: '2',
                       innerHTML: '<b>'+nb+' itinéraire<br/>'+leg.duration.text +
  ...
  drawGPolyline: function(steps) {
    ...
    var layer = new OpenLayers.Layer.Vector('Itineraire', { sldUrl: '/sld/itineraries.xml' });
  ...
  buildFeaturesFromGRoute: function(steps) {
    ...
    for (i = 0; i < numSteps; i++) {
      ...
      (function(feature) {
         ...
         var dc = dojo.create
         , tr1 = dc('tr', { 'class': 'link' }, self.resultNode )
         , td2 = dc('td', { innerHTML: '- <b>'+(i + 1) + '</b> -<br/>' +
                            step.duration.text
                            .replace(/secondes/,'s')
                            .replace(/minutes/, 'min') + '<br/>' +
                            step.distance.text }, tr1)
  ...
  onRouteError: function(status) {
    ...
    dojo.place(geonef.jig.makeDOM(['tr', {}, ['td', {}, str]]), this.resultNode);
  },
