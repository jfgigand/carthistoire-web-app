               ...
               geonef.ploomap.layer.featureDetails.ChildMixin ],
{
  ...
  saveAttrs: function() {
    ...
    ;
    //console.log('save :-/', this, value, this.feature);
    ...
    //dojo.publish('jig/workspace/flash', [ "Enregistrement de l'élément... " ]);
    /*var self = this;
    var _cnt;
    _cnt = this.connect(this.saveStrategy, 'onCommit',
      function(response) {
        console.log('commit resp', response);
        self.disconnect(_cnt);
        var els = response.reqFeatures.map(function(f) { f.layer.getFeatureTitle(f); }).join(', ');
        if (response.success()) {
          dojo.publish('jig/workspace/flash',
          [ "Enregistrement terminé des éléments : "+els ]);
        } else {
          dojo.publish('jig/workspace/flash',
          [ "Enregistrement échoué des éléments : "+els ]);
        }
      });*/
