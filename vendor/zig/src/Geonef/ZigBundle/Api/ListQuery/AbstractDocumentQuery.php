...
abstract class AbstractDocumentQuery extends AbstractQuery
{
  ...
  public function saveDocumentAction()
  {
    ...
    $this->documentManager->flush();
    if (method_exists($doc, 'updatePropValidity')) {
      $this->response['valid'] = $doc->updatePropValidity($this->container);
    }
    $this->documentManager->flush();
    $this->documentManager->detach($doc);
    $this->response['value'] = $this->modelToArray($doc);
