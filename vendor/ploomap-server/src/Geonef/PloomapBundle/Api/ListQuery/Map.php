...
class Map extends AbstractDocumentQuery
{
  ...
  public function getMapStringAction()
  {
    ...
    $this->response['mapString'] = $map->msMapString;//getMapString($this->container);
  }
  ...
  public function getWmsInfoAction()
  {
    ...
    $this->response['srs'] = explode(' ', $msMap->getMetaData('wms_srs'));
    $this->response['extent'] = Geo::msRectToExtent($msMap->extent);
    $this->response['layers'] = $doc->getLayerNames($this->container, $msMap);
    $this->response['enabledLayers'] =
      $doc->getLayerNames($this->container, $msMap, true);
  ...
  protected function getExportUrls(MapDoc $map, mapObj $msMap)
  {
    ...
    return array
      ('PNG' => $map->getWmsMapUrl($this->container,
       ...
       ->generate('geonefPloomapMapSvg', array('id' => $map->getId()))
       );
    //$url . 'SERVICE=WMS'
  }
  ...
  public function getLegendDataAction()
  {
    ...
    $this->response['debug'] = $doc->indicatorStatistics;
  }
  ...
  public function clearBuildAction()
  {
    foreach ($this->request['uuids'] as $uuid) {
      ...
      //$this->documentManager->detach($map);
      if (isset($this->request['getMapString']) &&
          $this->request['getMapString']) {
        $map->build($this->container);
        //$this->documentManager->persist($map);
        //$this->documentManager->flush();
      } else {
        $this->documentManager->persist($map);
      }
      $this->response['mapString'] = $map->msMapString;
      $this->documentManager->flush();
      //$this->documentManager->detach($map);
    }
