	     ...
	     [dijit._Widget, dijit._Templated, geonef.jig.widget._AsyncInit],
{
  ...
  installList: function(data) {
    ...
    data.rows.forEach(
      function(rowData) {
    	odd = !odd;
    	var rowWidget = this.createRow(rowData);
  ...
  createRow: function(rowData) {
    //console.log('createRow', this, rowData);
    var RowClass = dojo.getObject(this.rowClassName),
    rowWidget = new RowClass(
      dojo.mixin({}, rowData, { listWidget: this }));
  ...
  onSelectionChange: function(row, state) {
    ...
    this.selCountNode.innerHTML = this.selectionCount;
